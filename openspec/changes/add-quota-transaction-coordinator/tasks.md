...
- [x] 1.1 Add `SubscriptionQuota` model to `apps/api/prisma/schema.prisma`
...
- [x] 1.2 Add `SubscriptionTransaction` model to `apps/api/prisma/schema.prisma`
...
- [x] 1.4 Seed initial user quotas for testing
...
- [x] 2.1 Create transaction type enum in `apps/api/src/enum/transaction.enum.ts`
...
- [x] 2.2 Define credit cost constants in `apps/api/src/constants/quota.constants.ts`
...
- [x] 3.1 Create `QuotaTransactionCoordinatorService` at `apps/api/src/service/quota-transaction-coordinator.service.ts`
  - Inject `PrismaService` dependency
  - Initialize logger

- [x] 3.2 Implement `preDeduct` method
...
- [x] 3.3 Implement `settle` method (renamed from `commit`)
...
- [x] 3.4 Implement `rollback` method
...
- [x] 3.5 Implement `getQuota` method (renamed from `getBalance`)
...
- [x] 4.1 Create custom exceptions in `apps/api/src/exception/`
...
- [x] 4.2 Add error messages to `apps/api/src/exception/error-message.ts`
...
- [x] 5.1 Inject `QuotaTransactionCoordinatorService` into `TaskService`
  - Add to constructor dependencies
  - Store as private readonly property

- [x] 5.2 Update `createNanoBananaProTask` method
...
- [x] 5.3 Handle quota errors gracefully
...
- [x] 6.1 Add logging to all quota operations
