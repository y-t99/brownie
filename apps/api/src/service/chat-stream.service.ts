...
export class ChatStreamService {
  ...
  async userChatAssistantMessageStream(
    ...
    if (
      chatMessage.status !== ChatMessageStatus.PROCESSING &&
      chatMessage.status !== ChatMessageStatus.PENDING
    ) {
    ...
    const taskResourceRelation =
      await this.prisma.taskResourceRelation.findFirst({
        where: {
    ...
    const subscribe = runs.subscribeToRun<typeof FrontOfficeAssiant>(
      (task.meta as unknown as TaskMeta).handle_id,
    );
        ...
        transform(chunk, controller) {
          try {
            if (chunk.type === "run") {
              ...
              const modelMessage = (chunk as { chunk: unknown })
